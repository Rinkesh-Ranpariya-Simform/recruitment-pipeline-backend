 ...
 * `candidate.repository.ts` and nowhere else (BE-3, AZ-2).
 *
 ...
 * handler inspects a fetched candidate and decides what to send (BE-2, FR-7.2).
 *
 * Handlers don't catch — Express 5 forwards a rejected promise to the error
 * middleware, which is what keeps a Prisma code off the wire (ERR-4).
 *
 * Input comes from `req.validatedParams`, `req.validatedQuery` and `req.body`,
 * never the raw `req.params` / `req.query`, which are un-coerced strings (BE-5).
 ...
 * this feature writes (AZ-8, SEC-8).
 *
...
/**
 * One endpoint, two response shapes, chosen by the caller's role (XFE-1).
 *
...
/**
 * The scoped by-id read (FR-5.1, FR-6.1).
 *
 ...
 * (FR-4.7, XFE-10), so the client writes it straight into its detail cache
 * without a refetch.
 */
