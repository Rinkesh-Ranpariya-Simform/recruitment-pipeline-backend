 ...
 * bypassing the assignment predicate the whole system rests on. The answer is
 * not a scoped `200` for them; it is `403`, so that there is nothing here to
 * bypass.
 *
 ...
 * selects. A candidate's is `where: { candidateUserId }` from the verified
 * token; a recruiter's has no per-row predicate at all, because a recruiter may
 * read every application. Anyone widening the recruiter guard is not granting a
 ...
 * their payload was also malformed.
 *
 ...
 * and the interviews feature's two are mounted below them. They live in those
 * modules rather than here so the module owning a set of rules also owns every
 * route that applies them; the resource is an application, which is why they
 * hang off this path.
 */
...
/**
 * The pipeline feature's three writes, on `/:applicationId/…`.
 *
 ...
 * bypass the assignment predicate the whole feature rests on.
 */
