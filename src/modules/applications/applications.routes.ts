...
/**
 * TWO routes, both `CANDIDATE`-only.
 *
 ...
 * 405. An unregistered route falls through to `notFound` and answers 404 like
 * any other path that does not exist (FR-6.8, EC-09, AC-B44). **The absence is
 * the guarantee**: with no by-id surface, there is no scoping rule on it that
 * could be forgotten. Do not add one without a spec change.
 */
