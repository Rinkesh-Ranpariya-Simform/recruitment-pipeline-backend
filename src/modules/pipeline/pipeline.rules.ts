...
/**
 * The stage progression rule, and nothing else.
 *
 ...
 * answer *"where is the stage progression rule enforced?"*, so it has to be
 * readable without following an import out of it. Everything here is a pure
 * function over two constants.
 *
 * The original sketch treats `hired` and `rejected` as stages. **The shipped
 * schema does not**: they live on `ApplicationStatus`, kept disjoint from
 * `PipelineStage` so that `status: ACTIVE, currentStage: REJECTED` is
 * unrepresentable without a CHECK constraint. So the single map that sketch
 * implies splits in two here — `ALLOWED_STAGE_TRANSITIONS` governs movement
 * along the pipeline, `ALLOWED_OUTCOMES` governs leaving it.
 */

/**
 * The canonical order of the live pipeline.
 *
 * Declared ONCE, here. Two things read it and neither re-derives it: the
 * densifier that gives every role all four stage cells in board order, and
 * `stagesSkipped` below. It is not the enum's declaration order by coincidence —
 * it is the order, and `PipelineStage`'s declaration is kept to match.
 */
...
/**
 * The stage graph. One step forward, no skips, no reversals.
 *
 ...
 *
 * Everything this map refuses is reachable only through an override, which
 * records who did it and why. That is the whole of "cannot skip a stage without
 * an explicit override".
 *
 * Typed `Record<PipelineStage, …>` and therefore **total over the enum**:
 * adding a stage to `PipelineStage` is a compile error here until every
 * transition out of it has been decided. A partial map would be a silent hole
 * that refuses every move from the new stage with no one having chosen that.
 */
...
/**
 * Which terminal outcomes are reachable from each live stage.
 *
 ...
 * leaves a reason and an actor behind it.
 *
...
/**
 * Is this move one the graph permits?
 *
...
/** Is this outcome one the current stage permits? */
...
/**
 * How many stages a move jumps over.
 *
 ...
 * answerable at read time without re-deriving the graph.
 *
